...
一个飞书话题群（THREAD 模式）。老板和 Agent 的共享工作空间。
...
graph TB
    ...
    subgraph Agent 系统
        C["规则过滤<br>硬规则"] --> D["分类Agent<br>轻量LLM"] --> E["工作台<br>飞书话题群"]
...
工作台本身就是短期记忆——正在进行的事、跟老板的对话、定时任务队列，全在飞书话题群记录里。飞书即持久化层。
...
4. **飞书原生能力复用**——定时消息 = 定时任务，话题群 = 工作台
5. **用户零门槛**——老板只需要在飞书工作台里说话
