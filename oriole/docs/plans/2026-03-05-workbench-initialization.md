...
工作台是 Oriole 一切功能的基础设施。在用户首次使用时，需要通过编程自动检测并创建工作台。整个过程对用户来说应该是零配置的——用户不需要手动建话题群、配机器人、设定时消息。

## 初始化触发

Agent 启动时，首先检查当前用户是否已有工作台。工作台信息存储在飞书侧（群描述中），不依赖本地状态：

1. 查找是否存在符合工作台特征的话题群（如群描述中包含特定标识）
2. 如果找到，提取工作台信息，开始工作
3. 如果未找到，进入初始化流程
...
**关于 `chatMode` 的选择**：工作台里的"事"以话题（thread）为载体，THREAD 模式使话题群天然以话题组织信息，与"事"的设计契合。
...
初始化流程应当是幂等的——如果中途失败（如网络中断），重新执行不会产生重复的话题群或机器人。

策略：
- 创建话题群前先查找是否已有工作台群（通过群描述中的标识判断）
